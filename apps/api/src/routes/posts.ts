...
posts.get('/', async c => {
  ...
  const query = locale ?
    'SELECT id, content, locale, created_at FROM posts WHERE locale = ? ORDER BY created_at DESC LIMIT ? OFFSET ?' :
    'SELECT id, content, locale, created_at FROM posts ORDER BY created_at DESC LIMIT ? OFFSET ?'
...
posts.post(
  '/', validateTurnstile, rateLimiter, zValidator('json', createPostSchema), async c => {
    ...
    const post = await c.env.DB.prepare(
      'INSERT INTO posts (id, content, locale) VALUES (lower(hex(randomblob(8))), ?, ?) RETURNING id, content, locale, created_at'
    )
